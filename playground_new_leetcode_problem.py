...
REPO_ROOT = Path(__file__).resolve().parents[0]  # file is in tools/, so parent.parent is repo root
ROOT_DIR = REPO_ROOT
...
def create_problem(slug: str, lang: str, title: str, url: str, diff: str, tags: str):
    folder_name = f"{slug}"
    target_base = ROOT_DIR / folder_name / lang
    md_path = ROOT_DIR / folder_name / "problem.md"
    ...
    print(f"Created problem at: {ROOT_DIR / folder_name}")
...
def interactive_flow():
    ...
    next_num = find_next_number(ROOT_DIR)
...
def main(argv):
    ...
    if args.num:
        ...
        num = find_next_number(ROOT_DIR)

    title = args.title or args.slug.replace("_", " ").title()
    ok = create_problem(slug, args.lang, title, args.url, args.diff, args.tags)
